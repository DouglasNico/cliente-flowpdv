...
## 23/09/2026 — Desktop, clientes, SVG e perfil do PDV
- `reconstruction.css`: remove margem herdada do campo de data; botões primários/secundários/WhatsApp com cursor pointer, borda, altura, hover e foco consistentes; cartões de clientes com slots de ações alinhados, filtros quebram em linhas no celular; perfil com colunas e quebra segura.
- `index.html`, `app.js`, `flow-icons.js`: ícones de interface substituídos por SVG autoral de traço e cor herdada (menus, ações, categorias, status e modais). Opções nativas e mensagens de texto ficam sem emoji decorativo porque não aceitam SVG. Valores de ícone guardados em dados continuam intactos e são mapeados apenas na renderização. Sem observador global ou nova dependência de rede.
- Cargo exibido como Administrador, mantendo `gerente` como valor persistido por compatibilidade com o PDV. Não criados novos níveis Admin/Super Admin.
- `manager-rules.js`: perfil prioriza `config.nomeEmpresa/nomeLoja`, `config.cnpj`, `config.chavePix`, `config.telefone` sincronizados do PDV, com licença como fallback de nome/documento. Removido `whatsappSuporte` da leitura de telefone da loja. Campos escapados ao renderizar.
- Leitura verificada: PDV `src/js/app.js` salva dados e aciona `enviarAlteracaoNuvem('configuracoes')`; `cloud-sync.js` envia `config`, Gestor recebe o backup. Não houve gravação/teste ponta a ponta na loja real.
- Módulos: comportamento existente mantido nesta rodada. UI autoriza pelo PIN, grava `config.modulos` e tenta espelhar `licencas.modulos`. Recomendação pendente: liberação comercial no Master, Gestor apenas exibe status; preferências operacionais separadas. Não afirmar restrição já implementada.
- Cache do PWA atualizado. 8 testes Node aprovados; revisão Chromium sintética em 4 viewports/60 registros sem overflow/erros, cursor e hover exercitados. Detector sem defeitos mecânicos, apenas avisos de tokens documentais. Evidência em `flowpdv-sistema/output/reconstrucao-web-20260923/gestor-desktop/`.
- Site oficial inspecionado: já usa SVG, nenhuma modificação necessária. Master e cardápio recebem migração dos ícones residuais em seus próprios commits. Sem alterar dados/nomes de clientes reais.
